...
func runTest(t *testing.T, test quploadTest) {
	t.Parallel()
	runTestCommon(t, test)
}

func runTestCommon(t *testing.T, test quploadTest) {
	...
	// Set the static qupload datastore
	th.Timeout = 1500 * time.Millisecond
	th.CreateTestDirs()
	defer th.EndTest()
...
type quploadTest func(test *testHelper)
